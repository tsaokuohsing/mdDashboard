    ...
    function InboxController($scope, $filter, $location, $state, $mdMedia, $mdBottomSheet, $stateParams, $mdDialog, $mdToast, emails, contacts) {
        ...
        function createEmailGroups() {
            ...
            if(emails.status === 200) {
                ...
                vm.emailGroups = [{
                    name: $filter('mddTranslate')('Today'),
                    from: moment().startOf('day'),
                    to: moment().endOf('day')
                },{
                    name: $filter('mddTranslate')('Yesterday'),
                    from: moment().subtract(1, 'days').startOf('day'),
                    to: moment().subtract(1, 'days').endOf('day')
                },{
                    name: $filter('mddTranslate')('Older'),
        ...
        function composeClick($event) {
            $mdDialog.show({
                ...
                targetEvent: $event,
                locals: {
                    title: $filter('mddTranslate')('Compose'),
                    email: {
                        to: [],
                        cc: [],
                        bcc:[],
            ...
            function emailCancel() {
                $mdToast.show(
                    $mdToast.simple()
                    .content($filter('mddTranslate')('Email canceled'))
        ...
        function sendEmail($event, email) {
            ...
            $mdToast.show(
                $mdToast.simple()
                .content($filter('mddTranslate')('Email to {{to}} sent.', {to: sentTo.join(', ')}))
                .position('bottom right')
                .hideDelay(3000)
            );
        }
        ...
        $scope.$on('deleteEmail', function($event, deleteEmail) {
            angular.forEach(vm.emailGroups, function(group) {
                ...
                if(null !== removeEmailIndex) {
                    ...
                    $mdToast.show(
                        $mdToast.simple()
                        .content($filter('mddTranslate')('DeleteD'))
                        .position('bottom right')
                        .hideDelay(3000)
                    );
                }
