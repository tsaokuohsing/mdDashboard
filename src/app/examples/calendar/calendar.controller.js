(function() {
    ...
    /* @ngInject */
    function CalendarController($scope, $rootScope, $mdDialog, $mdToast, $filter, $element, mddTheming, mddLayout, uiCalendarConfig) {
        var vm = this;
        ...
        vm.calendarOptions = {
            ...
            viewRender: function(view) {
                ...
                mddLayout.layout.contentClass = 'calendar-background-image background-overlay-static overlay-gradient-10 calendar-background-month-' + vm.currentDay.month();
            },
            ...
            eventClick: function(calEvent, jsEvent, view) { //eslint-disable-line
                ...
                .then(function(event) {
                    ...
                    if(angular.isDefined(event.deleteMe) && event.deleteMe === true) {
                        ...
                        uiCalendarConfig.calendars['mdDashboard-calendar'].fullCalendar('removeEvents', event._id);
                        // change toast message
                        toastMessage = 'Event Deleted';
                    }
                    else {
                        // update event
                        uiCalendarConfig.calendars['mdDashboard-calendar'].fullCalendar('updateEvent', event);
                    }

                    // pop a toast
                    $mdToast.show(
                        $mdToast.simple()
                        .content($filter('mddTranslate')(toastMessage))
        ...
        function addEvent(event, $event) {
            ...
            $mdDialog.show({
                ...
                locals: {
                    ...
                    event: {
                        title: $filter('mddTranslate')('New Event'),
                        allDay: false,
                        start: vm.currentDay,
                        end: inAnHour,
                        palette: 'cyan',
            ...
            .then(function(event) {
                ...
                $mdToast.show(
                    $mdToast.simple()
                    .content($filter('mddTranslate')('Event Created'))
                    .position('bottom right')
                    .hideDelay(2000)
                );
            });
        ...
        function createRandomEvents(number, startDate, endDate) {
            ...
            for(var x = 0; x < number; x++) {
                ...
                var randomPalette = pickRandomProperty(mddTheming.palettes);
                ...
                vm.eventSources[0].events.push({
                    ...
                    backgroundColor: mddTheming.rgba(mddTheming.palettes[randomPalette]['500'].value),
                    borderColor: mddTheming.rgba(mddTheming.palettes[randomPalette]['500'].value),
                    textColor: mddTheming.rgba(mddTheming.palettes[randomPalette]['500'].contrast),
