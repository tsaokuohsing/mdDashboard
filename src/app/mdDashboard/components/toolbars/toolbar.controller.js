(function() {
    ...
    /* @ngInject */
    function DefaultToolbarController($scope, $injector, $rootScope, $mdMedia, $state, $element, $filter, $mdUtil, $mdSidenav, $mdToast, $timeout, $document, mddBreadcrumbsService, mddSettings, mddLayout) {
        var vm = this;
        vm.breadcrumbs = mddBreadcrumbsService.breadcrumbs;
        vm.emailNew = false;
        vm.languages = mddSettings.languages;
        ...
        function switchLanguage(languageCode) {
            if($injector.has('$translate')) {
                ...
                .then(function() {
                    $mdToast.show(
                        $mdToast.simple()
                        .content($filter('mddTranslate')('Language Changed'))
        ...
        function hideMenuButton() {
            return mddLayout.layout.sideMenuSize !== 'hidden' && $mdMedia('gt-sm');
        }

        function toggleNotificationsTab(tab) {
            $rootScope.$broadcast('mddSwitchNotificationTab', tab);
